...
const clientConfig = {
    '82acd2c6': { diskSpace: 450 * 1024 * 1024 * 1024, isNC: true }, // nc 盒子，拥有 450GiB 硬盘
    'e1a4047a': { diskSpace: 200 * 1024 * 1024 * 1024, isNC: false }, // 非 nc 盒子，拥有 200GiB 硬盘
};
// 设置种子分类与允许的下载器
...
function addTorrent(vertexRSSContext, torrent, ncRecordStr) {

    const { size: rssTorrentSize, name: rssTorrentName, hash: rssTorrentHash, url: rssTorrentUrl, link: rssTorrentLink } = torrent;

    logger.debug(`完成读取ncRecordStr: ${ncRecordStr}`);

    const ncRecord = JSON.parse(ncRecordStr);
    ...
    for (const clientID in clientConfig) {
        ...
        // 如果客户端类型是 nc
        if (clientConfig[clientID].isNC) {
            // 如果 NC 记录中存在，而且无高速，则跳过
            if (ncRecord[clientID] && !ncRecord[clientID].has_high_speed) {
                logger.debug(`NC ${clientID} 无高速，跳过...`);
                continue;
            }
        }
...
(torrent) => {
    ...
    redis.get(`vertex:nc_record`)
        .then(ncRecordStr => {
            addTorrent(vertexRSSContext, torrent, ncRecordStr);
        })

        .catch(err => {
            logger.error(`从Redis读取数据时出错: ${err.message}`);
        });
    // 不使用vertex原生进种
    return false;
};